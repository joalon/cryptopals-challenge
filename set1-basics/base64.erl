...
string_to_hex([$a | Rest], Acc) ->
	string_to_hex(Rest, <<10, Acc/binary>>);

string_to_hex([$b | Rest], Acc) ->
	string_to_hex(Rest, <<11, Acc/binary>>);

string_to_hex([$c | Rest], Acc) ->
	string_to_hex(Rest, <<12, Acc/binary>>);

string_to_hex([$d | Rest], Acc) ->
	string_to_hex(Rest, <<13, Acc/binary>>);

string_to_hex([$e | Rest], Acc) ->
	string_to_hex(Rest, <<14, Acc/binary>>);

string_to_hex([$f | Rest], Acc) ->
	string_to_hex(Rest, <<15, Acc/binary>>);

string_to_hex([Int | Rest], Acc) ->
	{ParsedInt, _RestOfString} = string:to_integer([Int]),
